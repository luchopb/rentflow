    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <div id="infoContratoActivo" class="me-auto d-none"></div>
                <button type="button" id="btnVerContrato" class="btn btn-info me-2 d-none" onclick="verDetalleContrato()">Ver Contrato</button>
...
<script>
    ...
    function actualizarBotonCrearContrato(propiedad) {
        ...
        const btnVerContrato = document.getElementById('btnVerContrato');
        const infoContrato = document.getElementById('infoContratoActivo');

        // Ocultar ambos botones y la información del contrato por defecto
        btnCrearContrato.classList.add('d-none');
        btnVerContrato.classList.add('d-none');
        infoContrato.classList.add('d-none');

        // Mostrar el botón de crear contrato solo si la propiedad está disponible
        if (propiedad.estado === 'Disponible') {
            btnCrearContrato.classList.remove('d-none');
        }
        // Mostrar información del contrato y botón para ver detalles si tiene contrato activo
        else if (propiedad.estado === 'Alquilado' && propiedad.contrato_id) {
            btnVerContrato.classList.remove('d-none');
            infoContrato.classList.remove('d-none');
            infoContrato.innerHTML = `
            <div class="alert alert-info mb-0">
                <strong>Contrato activo</strong><br>
                Inquilino: ${propiedad.inquilino_actual || 'No especificado'}
            </div>
        `;
            // Guardar el ID del contrato en el botón para usarlo después
            btnVerContrato.setAttribute('data-contrato-id', propiedad.contrato_id);
        }
    }
    ...
    function verDetalleContrato() {
        const contratoId = document.getElementById('btnVerContrato').getAttribute('data-contrato-id');
        ...
        // Cargar el módulo de contratos y mostrar el detalle del contrato
        if (typeof cargarModulo === 'function') {
            cargarModulo('contratos', function() {
                // Una vez cargado el módulo de contratos, obtener y mostrar el detalle del contrato
                fetch('modules/contratos/actions.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: 'action=get&id=' + encodeURIComponent(contratoId)
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success && data.data) {
                            // Obtener información adicional de la propiedad e inquilino
                            fetch('modules/propiedades/actions.php', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                                body: 'action=get&id=' + encodeURIComponent(data.data.propiedad_id)
                            })
                            .then(response => response.json())
                            .then(propData => {
                                if (propData.success && propData.data) {
                                    ...
                                    const contratoCompleto = {
                                        ...data.data,
                                        propiedad_direccion: propData.data.direccion,
                                        galeria: propData.data.galeria,
                                        local: propData.data.local
                                    };
                                    ...
                                    fetch('modules/inquilinos/actions.php', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                                        body: 'action=get&id=' + encodeURIComponent(data.data.inquilino_id)
                                    })
                                    .then(response => response.json())
                                    .then(inqData => {
                                        if (inqData.success && inqData.data) {
                                            // Completar la información del contrato
                                            contratoCompleto.inquilino_nombre = inqData.data.nombre;
                                            contratoCompleto.inquilino_dni = inqData.data.documento;
                                            
                                            // Mostrar el detalle del contrato
                                            if (typeof mostrarDetalleContrato === 'function') {
                                                mostrarDetalleContrato(contratoCompleto);
                                            }
                                        }
                                    })
                                    .catch(() => alert('Error al cargar información del inquilino'));
                                }
                            })
                            .catch(() => alert('Error al cargar información de la propiedad'));
                        } else {
                            alert('No se pudo cargar la información del contrato');
                        }
                    })
                    .catch(() => alert('Error de red al cargar el contrato'));
            });
        }
    }
</script>